...
public class DateUtils {
	...
	//private final static long MONTHTIME = DAYTIME*30;
	//private final static long YEARTIME = MONTHTIME*12;
	...
	public static int getHour(){
		int hour = 0;
		Date now = null;
		final TextView[] temp = null;
		final GetInternetDate getInternetDate = new GetInternetDate();
		new Thread(new Runnable() {
			@Override
			public void run() {
				getInternetDate.execute(temp);
			}
		}).start();
		now = getInternetDate.getNow();
		if(now == null){
			now = new Date();
		}
